from flask import Flask, render_template, request, jsonify  # pyright: ignore[reportMissingImports, reportUnknownVariableType]  # pyright: ignore[reportMissingImports], Response, send_file, session
import subprocess
import os
import json
import shlex
...
for folder in [app.config['UPLOAD_FOLDER'], app.config['RESULTS_FOLDER'], app.config['PROFILES_FOLDER']]:
...
DEFAULT_CREDS = {'proxy_host': '', 'proxy_port': '', 'proxy_user': '', 'proxy_pass': '', 'auth_header': ''}
...
def save_creds(creds: Dict[str, str]) -> bool:
    try:
        with open(CREDS_FILE, 'w') as f:
            json.dump({k: v for k, v in creds.items() if k in DEFAULT_CREDS}, f, indent=2)
        return True
    except Exception as e:
...
def run_scan_process(cmd: List[str], scan_id: str, q: queue.Queue) -> None:
    try:
        ...
        q.put({'type': 'complete', 'scan_id': scan_id, 'return_code': p.returncode, 'output': '\n'.join(out), 'status': status})
...
def scan():
    ...
    try:
        ...
        history.insert(0, {'id': scan_id, 'target': target, 'timestamp': datetime.now().isoformat(), 'status': status, 'return_code': result.returncode, 'result_file': result_file})
        history = history[:100]
        save_history(history)
        tray.set_status(status.title())
        return jsonify({'output': output, 'error': result.stderr or '', 'return_code': result.returncode, 'scan_id': scan_id, 'status': status})
    except subprocess.TimeoutExpired:
        tray.set_status('Timeout')
        return jsonify({'error': 'Scan timed out (5 min limit)'}), 408
    except FileNotFoundError:
        return jsonify({'error': 'sqlmap.py not found. Configure SQLMAP_PATH env var.'}), 500
    except Exception as e:
...
def scan_stream():
    ...
    rq = queue.Queue()
    with scan_lock:
        active_scans[scan_id] = {'target': target, 'cmd': cmd, 'start_time': datetime.now().isoformat(), 'status': 'running', 'queue': rq}
    t = threading.Thread(target=run_scan_process, args=(cmd, scan_id, rq))
    t.daemon = True
    t.start()
    tray.set_status('Running')
    return jsonify({'scan_id': scan_id, 'message': 'Scan started', 'command': ' '.join(cmd)})
...
def scan_status(scan_id):
    with scan_lock:
        if scan_id in active_scans:
            ...
            return jsonify({'scan_id': scan_id, 'status': s['status'], 'target': s['target'], 'start_time': s['start_time']})
    return jsonify({'error': 'Scan not found'}), 404
...
def rerun_scan(scan_id):
    ...
    try:
        ...
        history.insert(0, {'id': sid, 'target': target, 'timestamp': datetime.now().isoformat(), 'status': status, 'return_code': result.returncode})
        save_history(history)
...
def export_results():
    ...
    if fmt == 'txt':
        ...
        content = json.dumps({'target': target, 'timestamp': datetime.now().isoformat(), 'tool': 'sqlmap', 'output': output_text}, indent=2)
    ...
    with open(fp, 'w') as f:
        f.write(content)
    return jsonify({'message': 'Exported', 'filename': fn, 'download_url': f'/api/export/download/{fn}'})
...
def upload_file():
    ...
    return jsonify({'message': 'Uploaded', 'filepath': fp, 'filename': fn, 'parsed': parsed})
...
def batch_upload_urls():
    ...
    urls = [u.strip() for u in t.split('\n') if u.strip() and not u.strip().startswith('#')]
    return jsonify({'message': f'Parsed {len(urls)} URLs', 'urls': urls, 'count': len(urls)})
...
def batch_scan():
    ...
    for url in urls:
        ...
        try:
            ...
            results.append({'url': url, 'status': 'success' if r.returncode == 0 else 'error', 'return_code': r.returncode, 'output': r.stdout[:2000]})
        except subprocess.TimeoutExpired:
    ...
    with open(rf, 'w') as f:
        for r in results:
            f.write(f"\n{'='*60}\nURL: {r['url']}\nStatus: {r['status']}\n{'='*60}\n{r['output']}\n")
    history = load_history()
    history.insert(0, {'id': bid, 'target': f"Batch ({len(urls)} URLs)", 'timestamp': datetime.now().isoformat(), 'status': 'batch', 'result_file': rf, 'results': results})
    save_history(history)
...
def google_dork_scan():
    ...
    if se != 'Google':
        return jsonify({'error': f'Sqlmap -g flag only works with Google search engine. "{se}" search is not supported for dorking.'}), 400
    cmd = ['python', sqlmap, '-g', dork]
    if os.path.basename(os.path.dirname(sqlmap)) == 'sqlmap' and os.path.exists(os.path.join(sqlmap, 'sqlmap.py')):
        cmd = ['python', os.path.join(sqlmap, 'sqlmap.py'), '-g', dork]
    elif not os.path.exists(sqlmap) and os.path.exists(os.path.join(os.path.dirname(sqlmap), 'sqlmap', 'sqlmap.py')):
        cmd = ['python', os.path.join(os.path.dirname(sqlmap), 'sqlmap', 'sqlmap.py'), '-g', dork]
    ...
    try:
        ...
        history = load_history()
        history.insert(0, {'id': scan_id, 'target': f"Dork: {dork}", 'timestamp': datetime.now().isoformat(), 'status': status, 'return_code': result.returncode, 'result_file': result_file})
        save_history(history)
        tray.set_status(status.title())
        return jsonify({'output': output, 'error': result.stderr or '', 'return_code': result.returncode, 'command': ' '.join(cmd), 'scan_id': scan_id, 'status': status})
    except subprocess.TimeoutExpired:
        tray.set_status('Timeout')
        return jsonify({'error': 'Dork scan timed out (10 min limit). Try fewer results.'}), 408
    except FileNotFoundError:
        return jsonify({'error': f'sqlmap not found at {sqlmap}. Set correct path in settings.'}), 500
    except Exception as e:
        tray.set_status('Error')
        return jsonify({'error': f'Error: {str(e)}'}), 500
...
def google_dork_multi_scan():
    ...
    se = d.get('search_engine', 'Google')
    if se != 'Google':
        return jsonify({'error': 'Only Google search engine is supported for dorking.'}), 400
    ss = load_settings()
    sqlmap = ss.get('sqlmap_path', SQLMAP_PATH)
...
def get_stats():
    h = load_history()
    return jsonify({'total': len(h), 'successful': len([x for x in h if x.get('status')=='success']), 'errors': len([x for x in h if x.get('status')=='error']), 'batches': len([x for x in h if x.get('status')=='batch'])})
...
def get_settings():
    ss = load_settings()
    return jsonify({**load_creds(), 'sqlmap_path': ss.get('sqlmap_path', SQLMAP_PATH), 'max_file_size_mb': app.config['MAX_CONTENT_LENGTH']//(1024*1024), 'max_batch_size': 50, 'version': '2.1.0'})

@app.route('/api/settings', methods=['POST'])
def save_settings():
    d = request.json or {}
    save_creds({'proxy_host': d.get('proxy_host',''), 'proxy_port': d.get('proxy_port',''), 'proxy_user': d.get('proxy_user',''), 'proxy_pass': d.get('proxy_pass',''), 'auth_header': d.get('auth_header','')})
    ss = load_settings()
...
def health_check():
    ...
    return jsonify({'status': 'healthy' if ok else 'degraded', 'sqlmap_found': ok, 'sqlmap_path': path, 'uptime': datetime.now().isoformat()})


if __name__ == '__main__':
    print("\n" + "="*60)
    print("  SQLMap GUI v2.1 - Advanced Penetration Testing")
    print("="*60)
